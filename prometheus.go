...
var (
	metricTxChanCapacity = prometheus.NewGauge(prometheus.GaugeOpts{
		Name: "metalbond_tx_chan_capacity",
		Help: "Current capacity of txChan per peer",
	})

	metricRxChanEventCapacity = prometheus.NewGauge(prometheus.GaugeOpts{
		Name: "metalbond_rx_chan_event_capacity",
		Help: "Current capacity of rx chan events",
	})

	metricRxChanDataUpdateCapacity = prometheus.NewGauge(prometheus.GaugeOpts{
		Name: "metalbond_rx_chan_data_update_capacity",
		Help: "Current capacity of rx chan data update",
	})
...
func RegisterMetrics(registerer prometheus.Registerer, gatherer prometheus.Gatherer) {
	registerMetricsOnce.Do(func() {
		...
		registerer.MustRegister(
			metricTxChanCapacity,
			metricRxChanEventCapacity,
			metricRxChanDataUpdateCapacity,
			metricTxChanDepth,
...
func SetMetricTxChanCapacity(capacity int) {
	metricTxChanCapacity.Set(float64(capacity))
}

func SetMetricRxChanEventCapacity(capacity int) {
	metricRxChanEventCapacity.Set(float64(capacity))
}

func SetMetricRxChanDataUpdateCapacity(capacity int) {
	metricRxChanDataUpdateCapacity.Set(float64(capacity))
}
