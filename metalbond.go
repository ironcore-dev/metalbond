...
func (m *MetalBond) PeerLastKeepaliveSent(addr string) (time.Time, error) {
	m.mtxPeers.RLock()
	defer m.mtxPeers.RUnlock()

	m.log().Tracef("PeerLastKeepaliveSent peer %s", addr)
	if _, exists := m.peers[addr]; exists {
		lastKeepaliveSent := m.peers[addr].GetLastKeepaliveSent()
		return lastKeepaliveSent, nil
	} else {
